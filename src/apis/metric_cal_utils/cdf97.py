...
def fwt97_2d(m, nlevels=1):
    ...
    for i in range(nlevels):
        m = fwt97(m, int(w), int(h))  # cols
        m = fwt97(m, int(w), int(h))  # rows
        w /= 2
        h /= 2
...
def iwt97_2d(m, nlevels=1):
    ...
    for i in range(nlevels):
        m = iwt97(m, int(w), int(h))  # rows
        m = iwt97(m, int(w), int(h))  # cols
        h *= 2
        w *= 2
...
if __name__ == "__main__":
    # Load image.
    im = Image.open("../src_img/steps_upward.jpg")  # Must be a single band image! (grey)
    im = im.convert('L')
