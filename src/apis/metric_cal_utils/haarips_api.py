...
def haarpsi_pytorch(img1_tensor, img2_tensor, L=1.0, rescale_to_match=True, device=None):
    """
    ...
    # Calculate HaarPSI using piq.haarpsi
    # piq.haarpsi expects input tensors to be in the range [0, 1] and data_range=1.0
    haarpsi_score = piq.haarpsi(img1_piq_format, img2_piq_format, data_range=1.0)
    return haarpsi_score.item()

def haarpsi_api_torch(recon_img_batch, gt_img_batch, device=None) -> float:
    """
    ...
    for idx in range(recon_img_batch.shape[0]):
        feature_val += haarpsi_pytorch(recon_img_batch[idx], gt_img_batch[idx], device=device)
...
if __name__ == '__main__':
    ...
    # Different sizes, L=255
    ref_large_t_255 = (torch.rand(150, 160, 3, device=current_device) * 255)
    dist_small_t_255 = (torch.rand(128, 128, 3, device=current_device) * 255)

    # Float [0,1] range, L=1.0
    ref_float_t_1 = torch.rand(128, 128, device=current_device, dtype=torch.float32)
    dist_float_identical_t_1 = ref_float_t_1.clone()
    dist_float_noisy_t_1 = torch.clamp(ref_float_t_1.clone() + torch.randn(128, 128, device=current_device) * 0.15, 0,
                                       1)



    if piq is not None:
        print("\n--- PyTorch HaarPSI Examples ---")
        haarpsi_id_t = haarpsi_pytorch(ref_gray_t_255, dist_gray_identical_t_255, L=255.0)
        print(f"Identical Grayscale (H,W, L=255): HaarPSI = {haarpsi_id_t.item():.4f}")  # Expected ~1.0
        haarpsi_noisy_t = haarpsi_pytorch(ref_gray_t_255, dist_gray_noisy_t_255, L=255.0)
        print(f"Noisy Grayscale (H,W, L=255): HaarPSI = {haarpsi_noisy_t.item():.4f}")
        haarpsi_chwc_id_t = haarpsi_pytorch(ref_color_hwc_t_255, dist_color_hwc_identical_t_255, L=255.0)
        print(f"Identical Color (H,W,C, L=255): HaarPSI = {haarpsi_chwc_id_t.item():.4f}")
        haarpsi_cnchw_id_t = haarpsi_pytorch(ref_color_nchw_t_255, dist_color_nchw_identical_t_255, L=255.0)
        print(f"Identical Color (N,C,H,W, L=255): HaarPSI = {haarpsi_cnchw_id_t.item():.4f}")
        haarpsi_ds_t = haarpsi_pytorch(ref_large_t_255, dist_small_t_255, L=255.0, rescale_to_match=True)
        print(f"Different Sizes (L=255): HaarPSI = {haarpsi_ds_t.item():.4f}")
        haarpsi_f_id_t = haarpsi_pytorch(ref_float_t_1, dist_float_identical_t_1, L=1.0)
        print(f"Identical Float (H,W, L=1.0): HaarPSI = {haarpsi_f_id_t.item():.4f}")  # Expected ~1.0
        haarpsi_f_t = haarpsi_pytorch(ref_float_t_1, dist_float_noisy_t_1, L=1.0)
        print(f"Noisy Float (H,W, L=1.0): HaarPSI = {haarpsi_f_t.item():.4f}")
    else:
        print("\nSkipping VIF and HaarPSI examples because 'piq' library is not installed.")
